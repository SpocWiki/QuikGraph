...
namespace QuikGraph.Tests.Algorithms.ShortestPath
{
    ...
    internal sealed class YenShortestPathsAlgorithmTests
    {
        ...
        public void Constructor_Throws()
        {
            ...
            var graphAdj = new AdjacencyGraph<TestVertex, EquatableTaggedEdge<TestVertex, double>>();
            Assert.Throws<ArgumentException>(
                () => graphAdj.CreateYenShortestPathsAlgorithm(vertex1, vertex2, int.MaxValue));

            graphAdj = new AdjacencyGraph<TestVertex, EquatableTaggedEdge<TestVertex, double>>();
            graphAdj.AddVertex(vertex1);
            Assert.Throws<ArgumentException>(
                () => graphAdj.CreateYenShortestPathsAlgorithm(vertex1, vertex2, int.MaxValue));

            graphAdj = new AdjacencyGraph<TestVertex, EquatableTaggedEdge<TestVertex, double>>();
            graphAdj.AddVertex(vertex2);
            Assert.Throws<ArgumentException>(
                () => graphAdj.CreateYenShortestPathsAlgorithm(vertex1, vertex2, int.MaxValue));

            graphAdj = new AdjacencyGraph<TestVertex, EquatableTaggedEdge<TestVertex, double>>();
            graphAdj.AddVertexRange([vertex1, vertex2]);

            AdjacencyGraph<TestVertex, EquatableTaggedEdge<TestVertex, double>> nullGraph = null;
            Assert.Throws<ArgumentNullException>(
                () => nullGraph.CreateYenShortestPathsAlgorithm(vertex1, vertex2, int.MaxValue));
            Assert.Throws<ArgumentNullException>(
                () => graphAdj.CreateYenShortestPathsAlgorithm(null, vertex2, int.MaxValue));
            Assert.Throws<ArgumentNullException>(
                () => graphAdj.CreateYenShortestPathsAlgorithm(vertex1, null, int.MaxValue));
            Assert.Throws<ArgumentNullException>(
            ...
            Assert.Throws<ArgumentNullException>(
                () => graphAdj.CreateYenShortestPathsAlgorithm(null, null, int.MaxValue));
            Assert.Throws<ArgumentNullException>(
                () => nullGraph.CreateYenShortestPathsAlgorithm(null, null, int.MaxValue));
            // ReSharper restore AssignNullToNotNullAttribute

            Assert.Throws<ArgumentOutOfRangeException>(
                () => graphAdj.CreateYenShortestPathsAlgorithm(vertex1, vertex2, 0));
            Assert.Throws<ArgumentOutOfRangeException>(
                () => graphAdj.CreateYenShortestPathsAlgorithm(vertex1, vertex2, -1));
            // ReSharper restore ObjectCreationAsStatement
        }
        ...
        public void GraphWithCycle()
        {
            ...
            SortedPath<char>[] paths = algorithm.Execute().ToArray();
        ...
        public void GraphWithMultiplePaths()
        {
            ...
            SortedPath<string>[] paths = algorithm.Execute().ToArray();
        ...
        public void GraphWithMultiplePaths_KShortest()
        {
            ...
            SortedPath<char>[] paths = algorithmK5.Execute().ToArray();
        ...
        public void MultipleRunMethods()
        {
            ...
            void RunYenAndCheck(YenShortestPathsAlgorithm<char> yen)
            {
                ...
                SortedPath<char>[] paths = yen.Execute().ToArray();
        ...
        public void SortedPathHashCode()
        {
            ...
            var path1 = new SortedPath<int>(edges);
            var path2 = new SortedPath<int>(edges);

            var path3 = new SortedPath<int>(
            [
                new EquatableTaggedEdge<int, double>(1, 2, 1.0),
                new EquatableTaggedEdge<int, double>(2, 3, 1.0),
                new EquatableTaggedEdge<int, double>(3, 4, 1.0)
        ...
        public void SortedPathEnumeration()
        {
            ...
            var path = new SortedPath<int>(edges);
            CollectionAssert.AreEqual(edges, path);

            CollectionAssert.IsEmpty(
                new SortedPath<int>(Enumerable.Empty<EquatableTaggedEdge<int, double>>()));
        }
    }
}